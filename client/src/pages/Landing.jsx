...
const CATEGORIES = [
  { name: "Madhubani",   state: "Bihar",              count: 124, icon: "🎨", desc: "Ancient folk paintings of love & mythology",    from: "#C1272D", to: "#FF6B6B" },
  { name: "Warli",       state: "Maharashtra",         count: 89,  icon: "🔮", desc: "Tribal geometric art in earthy whites",         from: "#6B3F2A", to: "#B07244" },
  { name: "Kalamkari",   state: "Andhra Pradesh",      count: 156, icon: "🖌️", desc: "Hand-drawn narratives on cloth and paper",      from: "#1A4A6E", to: "#2196A4" },
  { name: "Pottery",     state: "Pan India",           count: 203, icon: "🏺", desc: "Earth shaped by patient, skilled hands",        from: "#7B3A10", to: "#E8651A" },
  { name: "Pattachitra", state: "Odisha",              count: 78,  icon: "📜", desc: "Vivid cloth-based scroll paintings of Jagannath",from: "#5C1F7A", to: "#C044B0" },
  { name: "Weaving",     state: "Varanasi & Beyond",   count: 167, icon: "🧵", desc: "Heritage in every handloom thread",             from: "#1A237E", to: "#673AB7" },
  { name: "Sculpture",   state: "Rajasthan",           count: 91,  icon: "🗿", desc: "Stories carved in stone & wood across centuries",from: "#2E3B40", to: "#546E7A" },
  { name: "Folk Art",    state: "All India",           count: 312, icon: "🎭", desc: "Vibrant colours from every region of Bharat",   from: "#B45309", to: "#D97706" },
];

const STEPS = [
  { n: "01", icon: "📸", title: "Artist Uploads", desc: "Local artists upload their work directly with price, description and cultural story — no gallery, no agent." },
  { n: "02", icon: "🔍", title: "You Discover",   desc: "Browse thousands of authentic artworks from verified artists across 28 Indian states, all in one place." },
  { n: "03", icon: "🚀", title: "Buy Direct",     desc: "Purchase with UPI or cards via Razorpay. The artist ships directly to you. Full credit stays with the creator." },
];

const STATS = [
  { label: "Artists",   target: 500,   suffix: "+", icon: "🎨" },
  { label: "Artworks",  target: 2000,  suffix: "+", icon: "🖼️" },
  { label: "States",    target: 28,    suffix: "",  icon: "🗺️" },
  { label: "Delivered", target: 1200,  suffix: "+", icon: "📦" },
];
    ...
    <div className="min-h-screen bg-deep noise">
      ...
      <section className="relative min-h-screen flex flex-col items-center justify-center text-center overflow-hidden pt-16">
        ...
        <div className="relative z-10 max-w-4xl mx-auto px-6">
          ...
          <div className="inline-flex items-center gap-2 bg-saffron/10 border border-saffron/30 rounded-full px-4 py-2 text-saffron text-xs font-medium tracking-wider mb-6 animate-fade-up">
            ...
            INDIA\'S DIRECT ART MARKETPLACE
          </div>
          ...
          <p className="text-cream-muted text-base md:text-lg max-w-xl mx-auto mb-8 leading-relaxed animate-fade-up" style={{animationDelay:"0.3s"}}>
            Buy authentic Indian art <em className="text-cream">directly</em> from local artists.
            Madhubani, Warli, Kalamkari, Pottery and more — no middlemen, full artist credit.
          </p>
      ...
      <section className="max-w-7xl mx-auto px-6 py-20">
        <div className="text-center mb-12 reveal">
          <p className="text-saffron text-xs uppercase tracking-[3px] mb-3">28 states, one platform</p>
          <h2 className="font-display text-4xl md:text-5xl text-cream font-bold">Discover India\'s Art Forms</h2>
          <p className="text-cream-muted mt-3 max-w-xl mx-auto text-sm leading-relaxed">
            From the terracotta of Bengal to the lacquerware of Rajasthan — every art form finds its home on SURANG.
          </p>
        </div>
      ...
      <section className="bg-surface/40 py-20 border-y border-surface-3/40">
        <div className="max-w-5xl mx-auto px-6">
          <div className="text-center mb-14 reveal">
            <p className="text-saffron text-xs uppercase tracking-[3px] mb-3">Like Swiggy — but for art</p>
            <h2 className="font-display text-4xl md:text-5xl text-cream font-bold">Direct. Authentic. Fair.</h2>
          </div>
      ...
      <section ref={statsRef} className="py-20 max-w-5xl mx-auto px-6">
        <div className="reveal text-center mb-12">
          <h2 className="font-display text-4xl text-cream font-bold">Giving Credit Where It Belongs</h2>
          <p className="text-cream-muted mt-3 text-sm max-w-md mx-auto">
            Every rupee goes directly to the artist. Every artwork carries the creator\'s name, story and state.
          </p>
        </div>
      ...
      <section className="max-w-4xl mx-auto px-6 py-10 pb-20 text-center reveal">
        <div className="relative bg-surface-2 border border-surface-3 rounded-3xl p-10 overflow-hidden">
          ...
          <div className="relative z-10">
            <p className="text-saffron text-xs uppercase tracking-[3px] mb-3">For every local artist</p>
            <h2 className="font-display text-4xl md:text-5xl text-cream font-bold mb-4">
              Your art deserves<br/>to be <span className="text-saffron">seen & sold.</span>
            </h2>
            <p className="text-cream-muted text-sm leading-relaxed max-w-lg mx-auto mb-8">
              Whether you\'re in a city studio or a remote village — SURANG puts your artwork in front of buyers across India. You set your price. You ship directly. <strong className="text-cream">Zero commission from us.</strong>
            </p>
            <div className="flex flex-col sm:flex-row gap-3 justify-center">
              <Link to="/register?role=artist" className="btn-saffron text-base px-8 py-3">Start Selling Today</Link>
              <Link to="/explore"             className="btn-outline text-base px-8 py-3">Browse as Buyer</Link>
            </div>
          </div>
        </div>
      </section>
